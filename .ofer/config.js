...
const getConfig = (rootname) => {
  const dataTypeFiles = fs.readdirSync(path.resolve(rootname, './.ofer/data/dataType'));
  const dataObjs = dataTypeFiles.map(dataType => require(path.resolve(rootname, `./.ofer/data/dataType/${dataType}`)));
  ...
  const PATHS = {
    CODE_PATH: path.resolve(rootname, './code'),
    TOOL_PATH: path.resolve(rootname, './.ofer/tool.js'),
    FILE_HANDLER_PATH: path.resolve(rootname, './.ofer/IO/fileHandler'),
    DATA_HANDLER_PATH: path.resolve(rootname, './.ofer/data/dataHandler'),
    OUTPUT_HANDLER_PATH: path.resolve(rootname, './.ofer/IO/outputHandler'),
    TOOL_HANDLER_PATH: path.resolve(rootname, './.ofer/tool/toolHandler')
  }
  ...
  const injectConfig = fs.existsSync(path.resolve(rootname, './config.json')) ?
    require(path.resolve(rootname, './config.json')) :
    {};
